...
# ---------- main ----------
def build_non_auto_tables(yaml_path: str) -> pd.DataFrame:    
    rows = []
    ...
    for doc in iter_yaml_docs(yaml_path):
        ...
        non_auto_entities = [e for e in entities if not entity_contains_auto(e)]

        for e in non_auto_entities:
...
# Save
df.to_csv("non_auto_terms_table319.csv", index=False)
print("Saved non_auto_terms_table319.csv")